...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--csv-path",
        type=Path,
        default=SCRIPT_DIR / "output/childes_l1_dataset/childes_l1_dataset.csv",
        help="Path to the cleaned CSV produced by build_childes_l1_dataset.py.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=SCRIPT_DIR / "output/childes_l1_models",
        help="Directory where models, metrics, and figures are saved.",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def grouped_train_test_split(
    ...
    """Split rows so that no child appears in both train and test.

    Some labels in this dataset may be represented by only one child.  A grouped
    split cannot put such a label in both train and test.  We keep singleton
    child-label groups in training so every known class is learnable, and split
    the remaining children into train/test groups.
    """

    child_labels = df.groupby("child_id")["l1"].nunique()
    ...
    label_to_child_count = df.groupby("l1")["child_id"].nunique()
    singleton_labels = label_to_child_count[label_to_child_count == 1].index.tolist()
    singleton_child_ids = set(
        df.loc[df["l1"].isin(singleton_labels), "child_id"].unique().tolist()
    )

    singleton_df = df[df["child_id"].isin(singleton_child_ids)]
    splittable_df = df[~df["child_id"].isin(singleton_child_ids)]

    if splittable_df["child_id"].nunique() < 2:
        raise ValueError(
            "Need at least two non-singleton child IDs to create a grouped test set."
        )
    ...
    all_labels = set(df["l1"].unique().tolist())
    for train_idx, test_idx in splitter.split(
        splittable_df,
        groups=splittable_df["child_id"],
    ):
        candidate_train = pd.concat(
            [singleton_df, splittable_df.iloc[train_idx]],
            ignore_index=True,
        )
        ...
        if all_labels.issubset(train_labels) and test_labels.issubset(train_labels):
            selected_split = (train_idx, test_idx)
            break

    if selected_split is None:
        raise ValueError(
            "Could not create a grouped split where all test labels are present "
            "in training. Try a smaller --test-size."
        )
    ...
    overlapping_children = set(train_df["child_id"]).intersection(test_df["child_id"])
    if overlapping_children:
        raise RuntimeError(f"Grouped split failed; overlap: {overlapping_children}")

    split_info = SplitInfo(
        train_rows=len(train_df),
        test_rows=len(test_df),
        train_children=train_df["child_id"].nunique(),
    ...
    return train_df.reset_index(drop=True), test_df, split_info
...
def evaluate_classifier(
    ...
    figure_path = output_dir / "figures" / f"confusion_matrix_{model_name}.png"
...
def main() -> None:
    ...
    joblib.dump(label_encoder, args.output_dir / "models" / "label_encoder.joblib")
