...
# Add the project root to sys.path based on this file's location. This makes the
# script import shared config reliably from any working directory.
PROJECT_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from src.paths import HF_DATASET_PATH, GUIDE_CSV_PATH, PROCESSED_DATA_DIR, RAW_CHILDES_DIR
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory where CSV and JSONL outputs are saved.",
    )
    parser.add_argument(
        "--hf-dataset-dir",
        type=Path,
        default=HF_DATASET_PATH,
        help="Directory where the Hugging Face dataset folder is saved.",
    )
    parser.add_argument(
        "--min-utterance-words",
        type=int,
...
def save_outputs(df: pd.DataFrame, output_dir: Path, hf_dataset_dir: Path) -> None:
    ...
    #   dataset = load_from_disk("data/hf_dataset/childes_l1_dataset")
    dataset = Dataset.from_pandas(df, preserve_index=False)
    DatasetDict({"train": dataset}).save_to_disk(hf_dataset_dir)

    print("\nSaved outputs")
    print("=============")
    print(f"CSV: {csv_path}")
    print(f"JSONL: {jsonl_path}")
    print(f"Hugging Face dataset: {hf_dataset_dir}")
...
def main() -> None:
    ...
    print_statistics(df)
    save_outputs(df, args.output_dir, args.hf_dataset_dir)
